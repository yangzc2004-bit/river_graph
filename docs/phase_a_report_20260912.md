...
| 计划验收项 | 结果 |
|---|---|
| 能区分历史预测、新多种子指标、冲突文件、缺失文件 | 达成（A0 清单 + manifest + 冲突记录 + 内容哈希棘轮） |
| 不执行真实训练即可证明保存/缓存行为正确 | 达成（31 项 mock 回归，训练计数断言） |
...
| 缺失问题被明确列出 | 达成（A2 §6、A3 §8/§11、README「已知限制」） |

## 9. 两轮外部审查的修正（截至 2026-09-12）

阶段 A 交付后经两轮独立审查，共 14 项问题，均已复现并修正：

- **第一轮（9 项）**：同名冲突文件重复进入分析；HUC2 由站号前缀推断；`config_hash` 不含数据集/mask；单文件原子不等于整次运行一致；`--rebuild-predictions` 训练却不保存；bootstrap 抽的是「站点 × 划分」行；06438000 观测量被误作观测值；身份冲突退出码为 0；布局检查漏刻度标签。
- **第二轮（5 项）**：中断恢复把新预测静默归到旧配置；重建清单会放行未核验的新内容；一致性检查漏 `log_*`/`pbias`/`n`/单侧 NaN；dry-run 身份口径与真实运行不一致；分组汇总把行当站点。

修正明细见 `experiments/analysis/predictions_report_20260912.md` §11 与 `docs/run_gnn_prediction_storage.md`。测试从 43 项增至 **99 项**，其中 31 项是针对上述缺陷的专用回归。

**仍未闭合**：`river_graph.pdf` 与 README 不同源（构建脚本 gitignore、环境无 reportlab）。
